...
const refreshAccessToken = asyncHandler(async (req, res) => {
  //body used to check if user is using mobile app then instead of cookies we send it in body
  ...
  //if not matched generate new token
  const options = {
    httpOnly: true,
    secure: true,
  };
...
const updateAccountDetail = asyncHandler(async (req, res) => {
  ...
  //new : true return the new updated values in the database
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  if (!avatar.url) {
    throw new ApiError(401, "oops something went wrong while uploading");
  }
